<template>
  <el-card>
    ...
    />
    <el-alert
      title="Token 自动管理：登录成功后 Token 会自动保存到 .mi.token 文件，无需手动配置"
      type="success"
      show-icon
      :closable="false"
      style="margin-bottom: 20px"
    ...
    >
      <el-form-item label="小米账号">
        <el-input
          v-model="form.MI_USER"
          placeholder="your_account@example.com"
        />
      </el-form-item>
      <el-form-item label="小米密码">
        <el-input
          v-model="form.MI_PASS"
          type="password"
      ...
      <el-form-item label="默认设备">
        <el-select
          v-model="form.MI_DID"
          placeholder="留空则使用第一个设备"
          clearable
          style="flex: 1; margin-right: 8px"
          :loading="devicesLoading"
          no-data-text="暂无设备，请先保存账号配置后点击刷新"
        >
          <el-option
            v-for="d in devices"
            :key="d.deviceID"
      ...
      <el-form-item label="服务地址">
        <el-input
          v-model="form.MUSIC_API_BASE_URL"
      ...
      <el-form-item label="轮询间隔">
        <div style="width: 100%">
          <div>
            <el-input-number
              v-model="form.CONVERSATION_POLL_INTERVAL"
              :min="0.1"
              :max="60"
              :step="0.1"
              :precision="1"
      ...
      <el-form-item label="唤醒词列表">
        <div style="width: 100%">
          <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 8px">
            <el-tag
              v-for="(word, index) in form.WAKE_WORDS"
              :key="index"
              closable
              @close="removeWakeWord(index)"
            >
              {{ word }}
            </el-tag>
            <el-input
              v-if="showWakeWordInput"
      ...
      <el-form-item label="局域网 IP 段">
        <div style="width: 100%">
          <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 8px">
            <el-tag
              v-for="(network, index) in form.PROXY_LAN_NETWORKS"
              :key="index"
              closable
              @close="removeLanNetwork(index)"
            >
              {{ network }}
            </el-tag>
            <el-input
              v-if="showLanNetworkInput"
              ref="lanNetworkInputRef"
              v-model="newLanNetwork"
              size="small"
              style="width: 180px"
              placeholder="192.168.0.0/16"
              @blur="addLanNetwork"
              @keyup.enter="addLanNetwork"
            />
            <el-button
              v-else
              size="small"
              @click="showLanNetworkInput = true"
            >
              + 添加 IP 段
            </el-button>
          </div>
          <div class="el-form-item__explain">
            CIDR 格式的局域网 IP 段，如 192.168.0.0/16
          </div>
        </div>
      </el-form-item>
      <el-form-item>
...
async function addWakeWord(): Promise<void> {
...
function removeWakeWord(index: number): void {
  form.value.WAKE_WORDS.splice(index, 1)
}

async function addLanNetwork(): Promise<void> {
...
function removeLanNetwork(index: number): void {
