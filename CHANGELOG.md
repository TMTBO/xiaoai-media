...
#### 配置热重载
- **无需重启** - 更新配置后自动生效，无需手动重启服务
- **自动重启服务** - 对话监听器、播放监控器等服务自动应用新配置
- **回调机制** - 支持注册配置变更回调函数，模块可以响应配置变化
- **零停机** - 配置更新过程中服务持续运行，不会中断正在进行的操作
...
详细说明请查看：[docs/config/CONFIG_HOT_RELOAD.md](docs/config/CONFIG_HOT_RELOAD.md)
...
- **回调机制** - 支持注册配置变更回调函数，模块可以响应配置变化
- **零停机** - 配置更新过程中服务持续运行，不会中断正在进行的操作
- **开发友好** - 在开发模式下不会与 uvicorn 的自动重载冲突
- **统一 Logger** - 所有模块使用统一的 logger 实例，方便管理和热更新
...
详细说明请查看：
- [配置热重载文档](docs/config/CONFIG_HOT_RELOAD.md)
- [快速参考](docs/config/CONFIG_HOT_RELOAD_QUICK_REF.md)
- [统一 Logger 管理](docs/config/UNIFIED_LOGGER.md)
