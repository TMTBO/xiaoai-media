...
class PlaybackController:
    ...
    def _get_current_playlist_id(self, device_id: str) -> Optional[str]:
        ...
        Returns:
            播单 ID，如果没有则返回 None
        """
        return self._state_service.get(
            f"current_playlist_{device_id}"
        ) or self._state_service.get("current_playlist_default")
    ...
    async def _notify_status_change(self, device_id: str, status: dict):
        ...
        _log.debug(
            "通知 %d 个订阅者设备 %s 状态变化", len(self._status_callbacks), device_id
        )
    ...
    async def _progress_push_loop(self, device_id: str):
        ...
        try:
            ...
            while True:
                ...
                _log.debug(
                    "设备 %s 进度推送: %d/%d ms", device_id, current_position, duration
                )
    ...
    async def _play_next(self, device_id: str, playlist_id: str):
        ...
        try:
            _log.info(
                "准备播放下一曲: device_id=%s, playlist_id=%s", device_id, playlist_id
            )
            ...
            _log.info("自动播放下一曲成功: %s", result.get("item", {}).get("title", ""))
        except Exception as e:
