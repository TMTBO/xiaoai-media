...
class ConversationPoller:
    ...
    def __init__(self, poll_interval: float = 2.0):
        ...
        self.last_timestamp: dict[str, int] = self._state_service.get(
        ...
        try:
            ...
            for device in devices:
                ...
                try:
                    ...
                    _log.debug(
                        "设备 %s 获取到 %d 条对话记录", device_name, len(conversations)
                    )
    ...
    async def _process_conversations(self, device_id: str, conversations: list[dict]):
        ...
        for record in conversations:
            ...
            _log.debug(
                "对话记录: 时间戳=%d, 查询=%s, 回答=%s, 上次时间戳=%d",
                timestamp,
                query,
                answer,
                self.last_timestamp.get(device_id, 0),
            )
