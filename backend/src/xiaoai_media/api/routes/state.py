        ...
        async def status_callback(dev_id: str, status: dict):
            ...
            try:
                full_state = await _build_full_state(
                    dev_id, device=None, basic_status=status
                )
...
async def _get_initial_state(device_id: str | None) -> dict[str, Any]:
    ...
    try:
        ...
        return await _build_full_state(
            dev_id, device, basic_status, status.get("play_song_detail", {})
        )
    except Exception as e:
    ...
    try:
        ...
        if not current_song:
            ...
            if play_song_detail:
                ...
                _log.info(
                    "从设备状态获取歌曲信息: name=%s, singer=%s, album=%s, cover=%s",
        ...
        _log.error("构建完整状态失败: %s", e, exc_info=True)
        return {"error": str(e), "device_id": device_id, **basic_status}
