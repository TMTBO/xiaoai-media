...
class VoiceCommandService:
    ...
    async def execute_command(text: str, device_id: str | None = None) -> dict:
        ...
        if "列表循环" in text or "循环播放" in text:
            return await VoiceCommandService._handle_play_mode_command(
                "loop", device_id
            )
        if "单曲循环" in text:
            return await VoiceCommandService._handle_play_mode_command(
    ...
    async def _try_match_playlist(text: str, device_id: str | None) -> dict | None:
        ...
        try:
            ...
            for pid, playlist_idx in index.items():
                for vk in playlist_idx.voice_keywords:
                    if vk.lower() in text.lower():
                        ...
                        _log.info(
                            "Matched playlist by voice_keyword: %r -> %s", vk, pid
                        )
                        break
            ...
            result = await PlaylistService.play_playlist(
                matched_playlist_id,
                PlayPlaylistRequest(
                    device_id=device_id, start_index=start_index, announce=True
                ),
            )
    ...
    async def _handle_playlist_command(text: str, device_id: str | None) -> dict:
        ...
        try:
            ...
            playlist_keyword = re.sub(
                r"(播单|列表|有声书|播客)\s*$", "", playlist_keyword
            ).strip()
            ...
            for pid, playlist_idx in index.items():
                ...
                if playlist_keyword.lower() in playlist_idx.name.lower():
                    matched_playlist_id = pid
                    _log.info(
                        "Matched playlist by name: %r -> %s", playlist_keyword, pid
                    )
                    break
    ...
    async def _handle_chart_command(text: str, device_id: str | None) -> dict:
        ...
        try:
            ...
            songs_data = await MusicService.get_rank_songs(
                rank_id, plat, page=1, limit=50
            )
            songs_raw: list[dict] = songs_data.get("data", {}).get("list", []) or []

            if not songs_raw:
                raise HTTPException(
                    status_code=404, detail=f"排行榜 {chart_name} 没有歌曲"
                )

            # 创建临时播放列表
            temp_playlist_id = (
                f"temp_chart_{plat}_{rank_id}_{int(datetime.now().timestamp())}"
            )
            playlist = PlaylistService.create_playlist(
                CreatePlaylistRequest(
                    name=f"{chart_name}（{plat}）",
                    type="chart",
                    description="临时排行榜播放列表",
                )
            )
    ...
    async def _handle_search_command(search_query: str, device_id: str | None) -> dict:
        ...
        try:
            ...
            # 搜索音乐
            search_data = await MusicService.search_music(
                search_query, plat, page=1, limit=50
            )
            songs_raw: list[dict] = search_data.get("data", {}).get("list", []) or []

            if not songs_raw:
                raise HTTPException(
                    status_code=404, detail=f"没有找到歌曲: {search_query}"
                )

            # 创建临时播放列表
            temp_playlist_id = (
                f"temp_search_{search_query}_{int(datetime.now().timestamp())}"
            )
            playlist = PlaylistService.create_playlist(
                CreatePlaylistRequest(
                    name=f"搜索: {search_query}",
                    type="search",
                    description="临时搜索结果播放列表",
                )
            )
            temp_playlist_id = playlist.id
    ...
    async def _handle_play_mode_command(mode: str, device_id: str | None) -> dict:
        ...
        try:
            PlaylistService.set_play_mode(
                current_playlist_id, PlayModeRequest(play_mode=mode)
            )
            ...
            # 播报模式变更
            await VoiceCommandService._announce_tts(
                f"已切换到{mode_name}模式", device_id
            )
